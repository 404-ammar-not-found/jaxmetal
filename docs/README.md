...
|---|---|---|
| [features/DF64_PRECISION.md](features/DF64_PRECISION.md) | v0.6.0 | Double-single ~48-bit arithmetic on a GPU with no `double` type. 1e7x more accurate than f32 on cancelling work, and 1.65-1.88x FASTER than CPU float64 when resident above ~4M elements. |
| [features/BATCHED_SOLVE.md](features/BATCHED_SOLVE.md) | v0.5.0 | Thousands of independent 3x3-8x8 solves, one GPU thread per system, LU with partial pivoting. Up to 14x a scalar C loop when resident, plus an SPD Cholesky path worth a further 2.7x (2-3x through the numpy-facing copy path), 15-24x over numpy batched. |
...
| [features/DEVICE_ROUTING.md](features/DEVICE_ROUTING.md) | v0.2.0, v0.7.0 | `device="auto"` for the MLP and the scientific ops. Picks GPU or CPU from measured crossovers, and accounts for residency — which moves the df64 threshold by ~50x. |
